...
export default async function BookmarksPage() {
  ...
  return (
    <div className="space-y-8">
      ...
      {totalCount === 0 ? (
        <div className="rounded-card border border-border-subtle bg-card p-12 text-center shadow-sm">
          <div className="mx-auto mb-4 flex h-12 w-12 items-center justify-center rounded-full bg-streak-warm/10 text-streak-warm">
              ...
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {lessons.map((lesson) => {
                  ...
                  return (
                    <div
                      key={lesson.id}
                      className="group relative flex flex-col justify-between rounded-card border border-border-subtle bg-card p-5 shadow-sm transition-[background-color,border-color,color,box-shadow,opacity,transform] hover:border-border hover:shadow-md"
                    >
            ...
            <div className="space-y-4">
              ...
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {flashcards.map((card) => (
                  <div
                    key={card.id}
                    className="flex flex-col justify-between rounded-card border border-border-subtle bg-card p-5 shadow-sm"
                  >
                    <div className="space-y-3">
                      ...
                      <div className="rounded-control bg-muted/50 p-3 text-xs text-muted-foreground">
                        {card.back}
                      </div>
                    </div>
                  </div>
            ...
            <div className="space-y-4">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                {questions.map((q) => (
                  <div
                    key={q.id}
                    className="flex flex-col justify-between rounded-card border border-border-subtle bg-card p-5 shadow-sm space-y-3"
                  >
                    <div className="flex items-center justify-between text-xs text-muted-foreground">
                    ...
                    <div className="text-xs text-muted-foreground bg-muted/30 p-2.5 rounded-control border border-border-subtle">
