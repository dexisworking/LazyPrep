...
export function CheckpointMocktest({
  ...
  return (
    <div className="space-y-6">
      ...
      {!result && (
        <div className="rounded-card border border-border-subtle bg-card p-4 text-sm text-muted-foreground">
      ...
      <div className="space-y-4">
        {questions.map((q, qi) => {
          ...
          return (
            <div key={q.id} className="space-y-3 rounded-card border border-border-subtle bg-card p-5">
                ...
                {q.options.map((opt, oi) => {
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        "flex w-full items-center gap-3 rounded-control border px-3 py-2.5 text-left text-sm transition-[background-color,border-color,color,box-shadow,opacity,transform]",
                        !r && isChosen && "border-primary bg-primary/10",
                        !r && !isChosen && "border-border bg-background hover:border-primary/40",
                        isCorrect && "border-np-success bg-np-success/10",
                        isWrongPick && "border-destructive bg-destructive/10",
                        r && !isCorrect && !isWrongPick && "border-border-subtle opacity-60",
                      )}
                    >
                      <span
                        className={cn(
                          "flex h-5 w-5 flex-shrink-0 items-center justify-center rounded-full border text-2xs font-semibold",
              ...
              {r && (
                <p className="ml-9 rounded-control border border-border-subtle bg-secondary/40 p-3 text-xs text-muted-foreground">
        ...
        <div className="flex items-center justify-between gap-4">
          ...
          <div className="flex flex-col items-end gap-1">
            ...
            <button
              ...
              className="inline-flex items-center gap-2 rounded-control bg-primary px-6 py-2.5 text-sm font-semibold text-primary-foreground transition-[background-color,border-color,color,box-shadow,opacity,transform] hover:opacity-90 disabled:opacity-50"
            >
