...
export function AdaptiveCourse({
  ...
  return (
    <div className="space-y-5">
      ...
      {phases.map((phase, i) => {
        const levelCls = LEVEL_BADGE[phase.phaseLevel ?? ""] ?? "border-border-subtle bg-secondary text-muted-foreground";
        ...
        return (
          <section
            ...
            className={
              ...
              (phase.locked ? "border-border-subtle bg-card/40 opacity-70" : "border-border-subtle bg-card")
            }
          >
            {/* Phase header */}
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div className="flex items-center gap-3">
                <div className="flex h-9 w-9 items-center justify-center rounded-control border border-border-subtle bg-secondary text-sm font-bold text-muted-foreground">
                  {i + 1}
                </div>
                <div>
                  <h2 className="flex items-center gap-2 text-lg font-semibold text-foreground">
                    {phase.title}
                    <span className={`rounded-full border px-2 py-0.5 text-3xs font-medium uppercase tracking-wide ${levelCls}`}>
                      {phase.phaseLevel}
                    </span>
                  </h2>
              ...
              {phase.checkpoint?.passed ? (
                ...
                <span className="inline-flex items-center gap-1.5 rounded-full border border-border-subtle bg-secondary px-3 py-1 text-xs font-medium text-muted-foreground">
            ...
            {phase.locked ? (
              ...
              <div className="mt-4 space-y-4">
                ...
                <div className="h-1.5 w-full overflow-hidden rounded-full bg-secondary">
                  <div className="h-full rounded-full bg-primary transition-[background-color,border-color,color,box-shadow,opacity,transform]" style={{ width: `${pct}%` }} />
                </div>
                ...
                <div className="space-y-3">
                  {phase.chapters.map((ch) => (
                    <div key={ch.id}>
                      ...
                      <ul className="overflow-hidden rounded-card border border-border-subtle">
                        {ch.lessons.map((l) => (
                          <li key={l.id} className="border-b border-border-subtle last:border-0">
                            <Link
                ...
                {/* checkpoint */}
                <div className="rounded-card border border-border-subtle bg-secondary/30 p-4">
                  {phase.checkpoint?.passed ? (
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-3">
                      ...
                      <Link
                        ...
                        className="inline-flex items-center gap-2 rounded-control bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground transition-[background-color,border-color,color,box-shadow,opacity,transform] hover:opacity-90"
                      >
